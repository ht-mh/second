...
$error = [];
$db = dbconnect();
$sql = 'SELECT * FROM shopping_list WHERE id=? and member_id=?  limit 1';
        $stmt = $db->prepare($sql);
        if(!$stmt){
            die('エラーだぴょん');
        }
...
<div id="content">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            <dl>
                ...
                <?php } ?>
                <input type="text" name="item_name" size="35" maxlength="20" 
                value="<?php if($form['item_name']){
                    echo h($form['item_name']);
                }else{
                    echo h($result['item_name']);
                }?>"/>  
                </dd>
                ...
                <dd>
                    <select name="shopping_year" size="2">
                        <?php $test='value="2022"'; ?>
                        <option <?php $test ?>selected>2022</option>
            ...
            <div><input type="submit" value="編集する"/></div>
            <br /><br />
        </form>
